...
public final class SnowflakeUtil {
    ...
    private SnowflakeUtil(long workerId, long datacenterId) {
        if (workerId > maxWorkerId || workerId < 0) {
            throw new BizException(String.format("workerId 不能大于 %d 或小于 0", maxWorkerId));
        }
        if (datacenterId > maxDatacenterId || datacenterId < 0) {
            throw new BizException(String.format("datacenterId 不能大于 %d 或小于 0", maxDatacenterId));
        }
    ...
    public synchronized long nextId() {
        ...
        if (timestamp < lastTimestamp) {
            ...
            // 系统时钟回退超出容忍范围，抛出异常
                ...
                throw new BizException(String.format("系统时钟回拨 %d 毫秒，拒绝生成 ID", offset));
            }
        }
