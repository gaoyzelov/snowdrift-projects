...
- **Provider 端：** DEBUG 级别记录成功调用及其耗时（含调用方 IP），ERROR 级别记录失败调用

```log
// Consumer 端日志格式示例：
DEBUG Dubbo调用成功 [Consumer] OrderService.createOrder(), elapsed=45ms
ERROR Dubbo调用异常 [Consumer] OrderService.createOrder(), elapsed=1203ms, ...

// Provider 端日志格式示例：
DEBUG Dubbo服务成功 [Provider] OrderServiceImpl.createOrder(), caller=192.168.1.100, elapsed=42ms
ERROR Dubbo服务失败 [Provider] PaymentServiceImpl.pay(), caller=192.168.1.100, elapsed=..., ...
```
